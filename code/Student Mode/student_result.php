                 ...
                 <table class="" id="dynamic_field" border="1">
                      ...
                      if(sizeof($result)==sizeof($x))
                      {
                        for($i=0; $i<sizeof($result); )
                        {
                          $save="";
                          $new=$i;
                          for($c2=0; $c2<(sizeof($var)-1); $c2++)
                          {

                            if($new % (sizeof($var)-1)==0)
                            {
                              echo "<tr>";
                              echo "<td><div class='example'>$y[$c]</div></td>";
                              if($result[$new] == $x[$new])
                              {
                                echo "<td><div class='example_t' >$result[$new] &#10003</div></td>";
                              }
                              else
                              {
                                $flag=1;
                                echo "<td><div class='example_f' title='The right answer is $result[$new]'>$x[$new] &#10007;</div></td>";
                              }
                              //$c++;
                              $save=$x[$new];
                            }
                          ...
                          {
                            $count_w++;
                            $query_insert = "INSERT INTO Error (studentname, filename, errorline, studentinput) VALUES ('$u', '$a', '$y[$c]', '$save')";
...
<?php
    ...
    $query_insert = "INSERT INTO `Score` (`filename`, `studentname`, `score`, `class`) VALUES ('$a', '$u', '$mark', '$class')";
