...
<?php
$query = "SELECT `username`, `password`, `symbol` FROM `clientlist`";
 ...
 if(!empty($_POST['username'])&& !empty($_POST['password']))
 {
   ...
   while($query_row = @mysql_fetch_assoc($query_run))
   {
     ...
     if($username == $_POST['username'] && $password == $_POST['password'] && $symbol == 's')
     {
       if($password==1)
       {
         header('Refresh: 0; URL = student_password.php');
       }
       else {
     ...
     {
       header('Refresh: 0; URL = ../teacher/upload/index.php');
     }
   }
 }
 ?>
...
<br>
<div><a href="register.php">Change password Here...</a></div>
</form>
